...
fn setup_with_make(svm: &mut LiteSVM, maker: &Keypair) -> EscrowCtx {
    ...
    let maker_ata_a =
        create_ata_and_mint_to(svm, maker, &mint_a.pubkey(), &maker.pubkey(), maker_initial);
...
fn test_take() {
    ...
    assert_eq!(
        maker_b_state.amount, ctx.receive,
        "maker did not get mint_b"
    );
